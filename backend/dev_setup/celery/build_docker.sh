...
docker create --mount type=bind,source="$(pwd)/src",target=/explori-celery/src/,readonly --mount type=bind,source="$(pwd)/cache",target=/explori-celery/cache/ --name explori-celery explori-celery
